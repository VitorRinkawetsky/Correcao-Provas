...
describe('dados usados nas telas existentes', () => {
    ...
    it('mantém a ordem e os valores das questões', () => {
        ...
        expect(exam.questions.map((question) => question.order)).toEqual([1, 2, 3, 4, 5, 6, 7, 8]);
        expect(exam.questions.map((question) => question.score)).toEqual([1.25, 1.25, 1.25, 1.25, 1.25, 1.25, 1.25, 1.25]);
    });
...
describe('listagem de provas', () => {
    it('lista todas as provas com contagem de questões e status', () => {
        ...
        expect(p1.questionCount).toBe(8);
